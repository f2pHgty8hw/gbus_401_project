...
foreach i of varlist yhat_*_split_* {
	...
	qui egen rmse = mean((admit - `i')^2) if missing(`i')==0
	replace rmse = sqrt(rmse)
	qui replace rmse = - rmse
	qui sum rmse if missing(`i')==0
	local rmse = r(mean)
	...
	*Calculate
	...
	mat cv_metrics = [`rmse', `r2', `mae', `accuracy', `log_loss']
...
ren cv_metricsc1 rmse
la var rmse "Negative mean root squared error"

ren cv_metricsc2 r2
la var r2 "R-squared"

ren cv_metricsc3 mae
la var mae "Negative mean absolute error"

ren cv_metricsc4 accuracy
la var accuracy "Accuracy"

ren cv_metricsc5 log_loss
la var log_loss "Negative log loss (cross entropy)"
